...
// === Workspace Switching Logic (NEWLY ADDED) ===
const allWorkspaces = document.querySelectorAll('.workspace-item');

allWorkspaces.forEach(workspace => {
    workspace.addEventListener('click', (e) => {
        e.preventDefault(); // Prevent the link from navigating

        // 1. Update the UI to show which workspace is active
        allWorkspaces.forEach(item => item.classList.remove('active'));
        workspace.classList.add('active');

        // 2. Get the name of the clicked workspace
        const workspaceName = workspace.querySelector('span').textContent.trim();
        
        // 3. Send a message to the background script to perform the context switch
